...
'
    ...
    for VM_NAME in $(cat "${VM_INPUT}" | grep -v "#" | sed '/^$/d' | sed -e 's/^[[:blank:]]*//;s/[[:blank:]]*$//'); do
        ...
        #checks to see if the VM has any snapshots to start with
            ...
            VM_HAS_INDEPENDENT_DISKS_FLAG=0
            if [[ ! -z ${INDEP_VMDKS} ]] ; then
                VM_HAS_INDEPENDENT_DISKS=1
                for k in ${INDEP_VMDKS}; do
                    K_VMDK=$(echo "${k}" | awk -F "###" '{print $1}')
		    if [[ $VMDK_FILES_TO_BACKUP == "all" ]]; then
                        VM_HAS_INDEPENDENT_DISKS_FLAG=1
		    else
		    	testString=${VMDK_FILES_TO_BACKUP//$K_VMDK/ }
		        if [[ "${VMDK_FILES_TO_BACKUP}" != "${testString}" ]]; then
                            VM_HAS_INDEPENDENT_DISKS_FLAG=1
			fi
                    fi
                done
            fi
            logger "info" "Independent disks: value $VM_HAS_INDEPENDENT_DISKS flag $VM_HAS_INDEPENDENT_DISKS_FLAG"
            ...
            if [[ ${CONTINUE_TO_BACKUP} -eq 1 ]] ; then
                ...
                if [[ ${SNAP_SUCCESS} -ne 1 ]] ; then
                    ...
                    VMDK_FAILED=1
                elif [[ ${VM_HAS_INDEPENDENT_DISKS} -eq 1 && ${VM_HAS_INDEPENDENT_DISKS_FLAG} -eq 1 ]] ; then
