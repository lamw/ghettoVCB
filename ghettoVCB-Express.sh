#!/bin/sh
# =============================================================================
# ghettoVCB Backup & Restore Wrapper Script
# David Harrop 
# August 2025
#
# Description:
...
# 	  - Prompt to rename vm(s) and edit the final restore file prior to restore
#     - Cleans up orphan vmkfstools processes and /tmp/ghetto* files after script interruption
#
# Usage:
...
#   - Only run one instance of this script at a time (the script cleans up old vmkfstools procecesses on run)
# =========================================================================================================================
...
# Ensure required mode arguments are set
[ -z "$ARG_MODE" ] && usage
...
# Non-Persistent NFS Restore Mount
# Only runs if non persistent NFS enabled in ghettoVCB.conf  (ignores any extra comments in ghettoVCB.conf
ENABLE_NON_PERSISTENT_NFS=$(grep -E '^ENABLE_NON_PERSISTENT_NFS=' "$GHB_CONF" | sed 's/#.*//;s/^.*=//;s/^"//;s/"$//;s/^[[:space:]]*//;s/[[:space:]]*$//')
...
if [ "$ENABLE_NON_PERSISTENT_NFS" = "1" ]; then
    ...
    UNMOUNT_NFS=$(grep -E '^UNMOUNT_NFS=' "$GHB_CONF" | sed 's/#.*//;s/^.*=//;s/^"//;s/"$//;s/^[[:space:]]*//;s/[[:space:]]*$//')
    NFS_SERVER=$(grep -E '^NFS_SERVER=' "$GHB_CONF" | sed 's/#.*//;s/^.*=//;s/^"//;s/"$//;s/^[[:space:]]*//;s/[[:space:]]*$//')
    NFS_MOUNT=$(grep -E '^NFS_MOUNT=' "$GHB_CONF" | sed 's/#.*//;s/^.*=//;s/^"//;s/"$//;s/^[[:space:]]*//;s/[[:space:]]*$//')
    NFS_LOCAL_NAME=$(grep -E '^NFS_LOCAL_NAME=' "$GHB_CONF" | sed 's/#.*//;s/^.*=//;s/^"//;s/"$//;s/^[[:space:]]*//;s/[[:space:]]*$//')
    NFS_VM_BACKUP_DIR=$(grep -E '^NFS_VM_BACKUP_DIR=' "$GHB_CONF" | sed 's/#.*//;s/^.*=//;s/^"//;s/"$//;s/^[[:space:]]*//;s/[[:space:]]*$//')
    ...
    if ! esxcli storage nfs list | grep -q "$NFS_LOCAL_NAME"; then
